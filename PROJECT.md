...
| `/admin/dashboard` | **Admin-only** (metrics + controls) | `app/(admin)/admin/dashboard/page.tsx` |
| `/credits` | **Protected** | `app/(dashboard)/credits/page.tsx` |
| `/referral` | **Protected** (referral page) | `app/(dashboard)/referral/page.tsx` |
...
Two-tab page at `app/(dashboard)/credits/`:

| Tab | Content |
|---|---|
| Buy Credits | Pro upgrade card + top-up packages (read from `admin_config`); each triggers Razorpay checkout inline |
| Usage History | Paginated `credit_transactions` list (20/page); color-coded by type |

A referral banner at the top links to `/referral`.
...
Dedicated referral page at `/referral` (`app/(dashboard)/referral/`):
- "How it works" 3-step guide, referral code copy, shareable link with Copy + WhatsApp share
- Stats: total referrals count, total credits earned
- Referral list: date, referred user name, credits earned
- "Program paused" state when admin disables referrals

Linked from: sidebar nav (Gift icon), bottom nav More sheet, credits page banner.

`/signup?ref=CODE` triggers referral processing after account creation. The signup form also has a visible optional "Referral code" text input (hidden when program is disabled). URL param takes priority over manual input.

1. `processReferral(code, newUserId)` server action reads config from `admin_config`
2. Checks if referral program is enabled
3. Inserts into `referrals` table
4. +bonus_credits to referrer + log
5. +bonus_credits to new user (on top of signup bonus) + log
6. Sets `user_profiles.referred_by`
...
Live at `/admin/dashboard` (`app/(admin)/admin/dashboard/page.tsx`). First link in the admin top bar.

### Sections

| Section | Content |
|---|---|
| User Metrics | Total users, users this month, active (7d), pro subscribers, total revenue, revenue this month |
| Practice Metrics | Total questions attempted, questions today |
| Credit Economy | Credits in circulation, purchased today, spent today, referral credits total |
| Recent Transactions | Last 20 credit_transactions with user email, type badge, amount, description, Razorpay payment ID |
| Real-time Controls | Credit costs, free tier, pro tier, topup packages, referral settings — all save to `admin_config` |

### Server actions (`app/(admin)/admin/dashboard/actions.ts`)

| Action | Does |
|---|---|
| `getAdminMetrics()` | Returns all metric counts from DB |
| `getRecentTransactions(limit)` | Returns recent credit_transactions joined with user emails and payment IDs |
| `updateAdminConfig(key, value)` | Upserts admin_config row; revalidates `/admin/dashboard`, `/credits`, `/referral` |

## Dynamic Configuration (`admin_config`)

All pricing and feature flags are stored in the `admin_config` table (key-value, jsonb). Read via `lib/queries/config.ts`:

| Key | Consumed by | Controls |
|---|---|---|
| `credit_costs` | `practice/actions.ts` `submitAttempt` | Credits per question attempt |
| `topup_packages` | `/credits` page | Top-up package prices and credit amounts |
| `pro_tier` | `/credits` page | Pro subscription price and monthly credits |
| `free_tier` | Cron job | Monthly free credits, signup bonus |
| `referral` | `/referral` page, signup form, `processReferral` | Bonus credits, referrer/referred amounts, enabled flag |

Changes made in the admin dashboard take effect immediately — no deploy needed. The `deduct_credit` Postgres RPC accepts a dynamic amount parameter (migration `0007`).
